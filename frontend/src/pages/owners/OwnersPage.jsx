...
import OwnerViewFields from './OwnerViewFields';
...
/**
 * Pagina principal per a la gestio de propietaris.
 * Inclou el llistat principal i un panell lateral de detalls.
 */
export default function OwnersPage() {
  // ESTATS I API
  const [filters, setFilters] = useState({ page: 1, per_page: 10 });
  const { data: owners, loading, refresh, meta, deleteItem } = useGet('/owners', filters);
  ...
  // ACCIONS
  /**
   * Gestiona l'eliminacio d'un propietari.
   * @param {number} id - Identificador del propietari a esborrar.
   */
  ...
  return (
    <div className={PAGE_CONTAINER}>
      <div className="mb-4">
        <h1 className={PAGE_TITLE}>Owners Management</h1>
      </div>
      ...
      <div className={CONTENT_LAYOUT}>
        {/* LLISTA */}
        <div className={LIST_CONTAINER} role="region" aria-label="Llistat de propietaris">
          <div className={LIST_ITEMS} role="list">
            {loading ? <p className={LOADING_MSG} role="status">Loading...</p> : owners.map(owner => (
              <div key={owner.id} 
                onClick={() => setSelected(owner)}
                onKeyDown={(e) => e.key === 'Enter' && setSelected(owner)}
                role="listitem"
                tabIndex="0"
                aria-selected={selected?.id === owner.id}
                className={`${ITEM_BASE} ${selected?.id === owner.id ? ITEM_SELECTED : ITEM_HOVER}`}>

                <div>
                  <h4 className={ITEM_TITLE}>
                    {formatText(owner.owner_name, 'Unknown Owner')}
                    {!owner.is_active && <span className={BADGE_INACTIVE}>Inactive</span>}
                  </h4>
                  <p className={ITEM_SUBTITLE}>
                    {formatText(owner.department)} • {formatText(owner.email, 'No Email')}
                  </p>
                </div>

                <div className={BTN_GROUP}>
                  <button onClick={(e) => { e.stopPropagation(); setEditing(owner); }} className={BTN_EDIT} aria-label={`Editar dades de ${owner.owner_name}`}>Edit</button>
                  <button onClick={(e) => { e.stopPropagation(); handleDelete(owner.id); }} className={BTN_DEL} aria-label={`Esborrar propietari ${owner.owner_name}`}>Delete</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* DETALL DRETA */}
        <aside className={SIDE_PANEL} aria-labelledby="detail-panel-title">
          <h2 id="detail-panel-title" className={PANEL_TITLE}>{selected ? selected.owner_name : "Select Owner"}</h2>
          {selected ? <OwnerViewFields owner={selected} /> : <p className={EMPTY_MSG}>Click an owner to see details</p>}
        </aside>
      </div>

      <Modal title={editing?.id ? "Edit Owner" : "Add Owner"} isOpen={!!editing} onClose={() => setEditing(null)}>
        <OwnerFormFields 
