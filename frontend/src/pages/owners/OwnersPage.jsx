...
export default function OwnersPage() {
  ...
  const { data: owners, loading, refresh, meta } = useGet('/owners', filters);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert("Error deleting owner. They might have assigned devices.");
    }
  };
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1 bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden flex flex-col">
          <div className="divide-y divide-gray-100 min-h-[500px]">
            {loading ? <p className="p-10 text-center text-xs text-gray-300 uppercase">Loading...</p> :
              owners.map(owner => (
                <div key={owner.id} onClick={() => setSelected(owner)}
                  ...
                  <div>
                    <h4 className="text-sm font-bold flex items-center gap-2">
                      {/* Aquí també podem fer servir formatText si volem evitar noms buits */}
                      {formatText(owner.owner_name, 'Unknown Owner')}
                      {!owner.is_active && <span className="text-[8px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded uppercase tracking-wider">Inactive</span>}
                    </h4>
                    
                    {/* 2. APLICAR FORMATTER AQUÍ */}
                  ...
                  <div className="flex gap-4">
                    <button 
                        onClick={(e) => { e.stopPropagation(); setEditing(owner); }} 
        ...
        <div className="w-96 sticky top-4 bg-white border border-gray-200 rounded-lg p-4 shadow-sm h-fit">
          <h2 className="text-lg font-bold mb-8 border-b border-gray-100 pb-4 truncate">
            {selected ? selected.owner_name : "Select Owner"}
          </h2>
          {selected ? <OwnerViewFields owner={selected} /> : <p className="text-center py-20 text-xs text-gray-300 italic uppercase">Click an owner to see details</p>}
        </div>
      </div>

      <Modal
        title={editing?.id ? "Edit Owner" : "Add Owner"}
        isOpen={!!editing}
